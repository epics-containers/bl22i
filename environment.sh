...
echo "Loading IOC environment for BL22I ..."
...
export EC_K8S_NAMESPACE=i22-iocs
# the git repo for this beamline (or accelerator domain)
export EC_DOMAIN_REPO=git@github.com:epics-containers/bl22i.git
...
if [[ $(module avail k8s-i22 2>&1) == *"k8s"* ]] ; then
    module unload k8s-i22 2> /dev/null
    module load k8s-i22
    # set the default namespace for kubectl and helm (for convenience only)
    kubectl config set-context --current --namespace=i22-iocs
